...
NAVEGATION_MENU_CONFIG = [
    ...
    },
    {
        ...
        'commands': [
            {'fetch:knowns': [
                'Fetch all knows <id, name>', fetch_knowns, True]},
            {'fetch:ids': ['Fetch all id from known list', fetch_ids, True]},
            {'fetch:phones': [
                'Fetch all phone number from known list', fetch_phones, True]},
            {'fetch:emails': [
                'Fetch all emails from known list', fetch_emails, True]},
            {'fetch:photos': [
                'Fetch all id from known list', fetch_photos, True]},
            {'fetch:all': [
                'Fetch all emails from known list', fetch_all, True]},
        ]
    },
    {
        'title': 'Investigation',
        'commands': [
            {'invest:knowns-of': ['Fetch all id from known list',
                                  find_knowns_of, True]},
...
class NavegationMenu:
    ...
    def display_menu(self, terminal):
        ...
        terminal.write('Commands'.center(22) +
                       'Description'.center(22), Terminal.MessageType.SUCCESS)
        terminal.write(column_separator + column_separator,
                       Terminal.MessageType.LOG)
        ...
        for i, group in enumerate(self.menu):
            ...
            for command in group['commands']:
                ...
                terminal.write(command_key.ljust(22) +
                               command_description.ljust(22))
...
def help_action(t): return help_func(t)
