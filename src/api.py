...
class Request:
    ...
    def write_response(text, sufix=''):
        file_name = join_path(OUTPUT_REQUESTS_DIR,
                              '%sget_request[%s].json' % (sufix, time.time()))
        fr = open(file_name, 'w')
...
class Facebook:
    ...
    def write_access_token(self):
        ...
        try:
            ...
            terminal.set_message_type(Terminal.MessageType.LOG)
            terminal.write('[*] Your access token is stored in ' +
                           relative_path(PATH_COOKIE_ACCESS_TOKEN))
            # success
        except KeyError:
    ...
    def get_profile_picture(self, profile_id):
        url = '%s/%s/picture?access_token=%s&height=300' % (
            BASE_URL, profile_id, self.access_token())
        contents = None
        try:
